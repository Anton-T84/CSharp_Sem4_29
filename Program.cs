...
int[] randomArrayCreation(int length,int minRnd,int maxRnd)
{  //Метод создания массива с заданным размером и случайными числами в заданном диапазоне
    if (length < 0 ) 
    {
...
void printArrayNumb(int[] array)
{  //Метод печати массива - как набор цифр
...
{  //Метод печати массива - как массив
    for (int i =0 ; i < array.Length; i++)
    {
...
void FullPrint(int[] array)
{  //Метод объединения печати 2х вариантов
    Console.Write("\n");
    if (array.Length == 0 ) Console.WriteLine("\tПопробуйте еще раз");
    else 
    {
...
int[] fullRandomArray()
{  //Метод Создания случайного массива длиной от 5 до 50 и случайными цифрами от 0 до 999
...
void choicenButPrint(string but)
{  // метод описания что было выбрано
    if (but == "N") Console.WriteLine("\t Выбран вариант с указанием параметров нового массива ");
...
int[] arrayCreation()
{  // метод создания массива через список
...
int[] convertListToArray(List<int> list)
{  // метод создания массива через список
...
Console.WriteLine("Для выхода, нажмите ESCAPE, люба другая кнопка запустит программу повторно.");
} 
